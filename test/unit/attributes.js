...
test( "EhQuery.propFix integrity test", function() {
	expect( 1 );

	//  This must be maintained and equal EhQuery.attrFix when appropriate
	...
	deepEqual( props, EhQuery.propFix, "EhQuery.propFix passes integrity check" );
});

test( "attr(String)", function() {
	expect( 50 );

	equal( EhQuery("#text1").attr("type"), "text", "Check for type attribute" );
	equal( EhQuery("#radio1").attr("type"), "radio", "Check for type attribute" );
	equal( EhQuery("#check1").attr("type"), "checkbox", "Check for type attribute" );
	equal( EhQuery("#simon1").attr("rel"), "bookmark", "Check for rel attribute" );
	equal( EhQuery("#google").attr("title"), "Google!", "Check for title attribute" );
	equal( EhQuery("#mark").attr("hreflang"), "en", "Check for hreflang attribute" );
	equal( EhQuery("#en").attr("lang"), "en", "Check for lang attribute" );
	equal( EhQuery("#simon").attr("class"), "blog link", "Check for class attribute" );
	equal( EhQuery("#name").attr("name"), "name", "Check for name attribute" );
	equal( EhQuery("#text1").attr("name"), "action", "Check for name attribute" );
	ok( EhQuery("#form").attr("action").indexOf("formaction") >= 0, "Check for action attribute" );
	equal( EhQuery("#text1").attr("value", "t").attr("value"), "t", "Check setting the value attribute" );
	equal( EhQuery("#text1").attr("value", "").attr("value"), "", "Check setting the value attribute to empty string" );
	equal( EhQuery("<div value='t'></div>").attr("value"), "t", "Check setting custom attr named 'value' on a div" );
	equal( EhQuery("#form").attr("blah", "blah").attr("blah"), "blah", "Set non-existent attribute on a form" );
	equal( EhQuery("#foo").attr("height"), undefined, "Non existent height attribute should return undefined" );

	// [7472] & [3113] (form contains an input with name="action" or name="id")
	var extras = EhQuery("<input id='id' name='id' /><input id='name' name='name' /><input id='target' name='target' />").appendTo("#testForm");
	equal( EhQuery("#form").attr("action","newformaction").attr("action"), "newformaction", "Check that action attribute was changed" );
	equal( EhQuery("#testForm").attr("target"), undefined, "Retrieving target does not equal the input with name=target" );
	equal( EhQuery("#testForm").attr("target", "newTarget").attr("target"), "newTarget", "Set target successfully on a form" );
	equal( EhQuery("#testForm").removeAttr("id").attr("id"), undefined, "Retrieving id does not equal the input with name=id after id is removed [#7472]" );
	// Bug #3685 (form contains input with name="name")
	equal( EhQuery("#testForm").attr("name"), undefined, "Retrieving name does not retrieve input with name=name" );
	extras.remove();

	equal( EhQuery("#text1").attr("maxlength"), "30", "Check for maxlength attribute" );
	equal( EhQuery("#text1").attr("maxLength"), "30", "Check for maxLength attribute" );
	equal( EhQuery("#area1").attr("maxLength"), "30", "Check for maxLength attribute" );
	...
	equal( EhQuery("#tAnchor5").attr("href"), "#5", "Check for non-absolute href (an anchor)" );
	...
	EhQuery("#list-test").attr( "list", "datalist" );
	equal( EhQuery("#list-test").attr("list"), "datalist", "Check setting list attribute" );

	// Related to [5574] and [5683]
	var body = document.body, $body = EhQuery( body );
	...
	equal( EhQuery( option ).prop("selected"), true, "Make sure that a single option is selected, even when in an optgroup." );

	var $img = EhQuery("<img style='display:none' width='215' height='53' src='data/1x1.jpg'/>").appendTo("body");
	...
	var styleElem = EhQuery("<div/>").appendTo("#qunit-fixture").css({
	...
	var $button = EhQuery("<button>text</button>").insertAfter("#button");
	...
	equal( EhQuery("#table").attr("test:attrib"), undefined, "Retrieving a non-existent attribute on a table with a colon does not throw an error." );
	equal( EhQuery("#table").attr( "test:attrib", "foobar" ).attr("test:attrib"), "foobar", "Setting an attribute on a table with a colon does not throw an error." );

	var $form = EhQuery("<form class='something'></form>").appendTo("#qunit-fixture");
	equal( $form.attr("class"), "something", "Retrieve the class attribute on a form." );

	var $a = EhQuery("<a href='#' onclick='something()'>Click</a>").appendTo("#qunit-fixture");
	equal( $a.attr("onclick"), "something()", "Retrieve ^on attribute without anonymous function wrapper." );

	ok( EhQuery("<div/>").attr("doesntexist") === undefined, "Make sure undefined is returned when no attribute is found." );
	ok( EhQuery("<div/>").attr("title") === undefined, "Make sure undefined is returned when no attribute is found." );
	equal( EhQuery("<div/>").attr( "title", "something" ).attr("title"), "something", "Set the title attribute." );
	ok( EhQuery().attr("doesntexist") === undefined, "Make sure undefined is returned when no element is there." );
	equal( EhQuery("<div/>").attr("value"), undefined, "An unset value on a div returns undefined." );
	strictEqual( EhQuery("<select><option value='property'></option></select>").attr("value"), undefined, "An unset value on a select returns undefined." );

	$form = EhQuery("#form").attr( "enctype", "multipart/form-data" );
...
test( "attr(String) in XML Files", function() {
	...
	equal( EhQuery( "locations", xml ).attr("class"), "foo", "Check class attribute in XML document" );
	equal( EhQuery( "location", xml ).attr("for"), "bar", "Check for attribute in XML document" );
	equal( EhQuery( "location", xml ).attr("checked"), "different", "Check that hooks are not attached in XML document" );
});
...
test( "attr(String, Function)", function() {
	...
	equal(
		EhQuery("#text1").attr( "value", function() {
			return this.id;
		}).attr("value"),
		"text1",
		"Set value from id"
	...
	equal(
		EhQuery("#text1").attr( "title", function(i) {
			return i;
		}).attr("title"),
		"0",
...
test( "attr(String, Object)", function() {
	...
	var div = EhQuery("div").attr("foo", "bar"),
		i = 0,
		fail = false;
	...
	EhQuery("#name").attr( "name", "something" );
	equal( EhQuery("#name").attr("name"), "something", "Set name attribute" );
	EhQuery("#name").attr( "name", null );
	equal( EhQuery("#name").attr("name"), undefined, "Remove name attribute" );

	var $input = EhQuery( "<input>", {
		name: "something",
		id: "specified"
	});
	...
	$input = EhQuery("<input type='checkbox'/>").attr( "checked", true );
	...
	$input = EhQuery("<option/>").attr( "selected", true );
	...
	$input = EhQuery("#text1").prop( "readOnly", true ).prop( "readOnly", false ).attr( "readonly", true );
	...
	$input = EhQuery("#check2").attr( "checked", true ).attr( "checked", false ).prop( "checked", true );
	...
	$input = EhQuery("#check2").attr( "checked", false ).attr( "checked", "checked" );
	equal( $input.attr("checked"), "checked", "Set checked to 'checked' (verified by .attr)" );

	var $radios = EhQuery("#checkedtest").find("input[type='radio']");
	...
	$input = EhQuery("#text1").attr( "readonly", false ).prop( "readOnly", true );
	...
	$input = EhQuery("#name").attr( "maxlength", "5" );
	...
	var $details = EhQuery("<details open></details>").appendTo("#qunit-fixture");
	...
	EhQuery("#foo").attr("contenteditable", true);
	equal( EhQuery("#foo").attr("contenteditable"), "true", "Enumerated attributes are set properly" );
	...
	EhQuery.each( [ commentNode, textNode, attributeNode ], function( i, elem ) {
		var $elem = EhQuery( elem );
	...
	EhQuery.each( [ window, document, obj, "#firstp" ], function( i, elem ) {
	...
	var table = EhQuery("#table").append("<tr><td>cell</td></tr><tr><td>cell</td><td>cell</td></tr><tr><td>cell</td><td>cell</td></tr>"),
	...
	equal( EhQuery("#area1").attr("value"), undefined, "Value attribute is distinct from value property." );

	// for #1070
	EhQuery("#name").attr( "someAttr", "0" );
	equal( EhQuery("#name").attr("someAttr"), "0", "Set attribute to a string of '0'" );
	EhQuery("#name").attr( "someAttr", 0 );
	equal( EhQuery("#name").attr("someAttr"), "0", "Set attribute to the number 0" );
	EhQuery("#name").attr( "someAttr", 1 );
	equal( EhQuery("#name").attr("someAttr"), "1", "Set attribute to the number 1" );
	...
	// Type
	var type = EhQuery("#check2").attr("type");
	try {
		EhQuery("#check2").attr( "type", "hidden" );
	...
	var thrown = true;
	try {
		EhQuery( check ).attr( "type", "checkbox" );
	} catch( e ) {
		thrown = false;
	}
	ok( thrown, "Exception thrown when trying to change type property" );
	equal( "checkbox", EhQuery( check ).attr("type"), "Verify that you can change the type of an input element that isn't in the DOM" );
	...
	EhQuery("#name").attr( "maxlength", "5" ).removeAttr("nonexisting");
	equal( typeof EhQuery("#name").attr( "maxlength", undefined ), "object", ".attr('attribute', undefined) is chainable (#5571)" );
	equal( EhQuery("#name").attr( "maxlength", undefined ).attr("maxlength"), "5", ".attr('attribute', undefined) does not change value (#5571)" );
	equal( EhQuery("#name").attr( "nonexisting", undefined ).attr("nonexisting"), undefined, ".attr('attribute', undefined) does not create attribute (#5571)" );
});
...
test( "attr(String, Object) - Loaded via XML document", function() {
	...
	EhQuery( "tab", xml ).each(function() {
		titles.push( EhQuery( this ).attr("title") );
	});
...
test( "attr('tabindex')", function() {
	...
	equal( EhQuery("#listWithTabIndex").attr("tabindex"), "5", "not natively tabbable, with tabindex set to 0" );
	equal( EhQuery("#divWithNoTabIndex").attr("tabindex"), undefined, "not natively tabbable, no tabindex set" );

	// anchor with href
	equal( EhQuery("#linkWithNoTabIndex").attr("tabindex"), undefined, "anchor with href, no tabindex set" );
	equal( EhQuery("#linkWithTabIndex").attr("tabindex"), "2", "anchor with href, tabindex set to 2" );
	equal( EhQuery("#linkWithNegativeTabIndex").attr("tabindex"), "-1", "anchor with href, tabindex set to -1" );

	// anchor without href
	equal( EhQuery("#linkWithNoHrefWithNoTabIndex").attr("tabindex"), undefined, "anchor without href, no tabindex set" );
	equal( EhQuery("#linkWithNoHrefWithTabIndex").attr("tabindex"), "1", "anchor without href, tabindex set to 2" );
	equal( EhQuery("#linkWithNoHrefWithNegativeTabIndex").attr("tabindex"), "-1", "anchor without href, no tabindex set" );
});

test( "attr('tabindex', value)", function() {
	expect( 9 );

	var element = EhQuery("#divWithNoTabIndex");
	...
	element = EhQuery("#linkWithTabIndex");
...
test( "removeAttr(String)", function() {
	...
	equal( EhQuery("#mark").removeAttr("class").attr("class"), undefined, "remove class" );
	equal( EhQuery("#form").removeAttr("id").attr("id"), undefined, "Remove id" );
	equal( EhQuery("#foo").attr( "style", "position:absolute;" ).removeAttr("style").attr("style"), undefined, "Check removing style attribute" );
	equal( EhQuery("#form").attr( "style", "position:absolute;" ).removeAttr("style").attr("style"), undefined, "Check removing style attribute on a form" );
	equal( EhQuery("<div style='position: absolute'></div>").appendTo("#foo").removeAttr("style").prop("style").cssText, "", "Check removing style attribute (#9699 Webkit)" );
	equal( EhQuery("#fx-test-group").attr( "height", "3px" ).removeAttr("height").get( 0 ).style.height, "1px", "Removing height attribute has no effect on height set with style attribute" );

	EhQuery("#check1").removeAttr("checked").prop( "checked", true ).removeAttr("checked");
	equal( document.getElementById("check1").checked, false, "removeAttr sets boolean properties to false" );
	EhQuery("#text1").prop( "readOnly", true ).removeAttr("readonly");
	equal( document.getElementById("text1").readOnly, false, "removeAttr sets boolean properties to false" );

	EhQuery("#option2c").removeAttr("selected");
	equal( EhQuery("#option2d").attr("selected"), "selected", "Removing `selected` from an option that is not selected does not remove selected from the currently selected option (#10870)" );

	try {
		$first = EhQuery("#first").attr( "contenteditable", "true" ).removeAttr("contenteditable");
	...
	$first = EhQuery("<div Case='mixed'></div>");
...
test( "removeAttr(Multi String, variable space width)", function() {
	...
	var div = EhQuery("<div id='a' alt='b' title='c' rel='d'></div>"),
		tests = {
			id: "a",
			alt: "b",
			title: "c",
...
test( "prop(String, Object)", function() {
	...
	equal( EhQuery("#text1").prop("value"), "Test", "Check for value attribute" );
	equal( EhQuery("#text1").prop( "value", "Test2" ).prop("defaultValue"), "Test", "Check for defaultValue attribute" );
	equal( EhQuery("#select2").prop("selectedIndex"), 3, "Check for selectedIndex attribute" );
	equal( EhQuery("#foo").prop("nodeName").toUpperCase(), "DIV", "Check for nodeName attribute" );
	equal( EhQuery("#foo").prop("tagName").toUpperCase(), "DIV", "Check for tagName attribute" );
	equal( EhQuery("<option/>").prop("selected"), false, "Check selected attribute on disconnected element." );

	equal( EhQuery("#listWithTabIndex").prop("tabindex"), 5, "Check retrieving tabindex" );
	EhQuery("#text1").prop( "readonly", true );
	equal( document.getElementById("text1").readOnly, true, "Check setting readOnly property with 'readonly'" );
	equal( EhQuery("#label-for").prop("for"), "action", "Check retrieving htmlFor" );
	EhQuery("#text1").prop("class", "test");
	equal( document.getElementById("text1").className, "test", "Check setting className with 'class'" );
	equal( EhQuery("#text1").prop("maxlength"), 30, "Check retrieving maxLength" );
	EhQuery("#table").prop( "cellspacing", 1 );
	equal( EhQuery("#table").prop("cellSpacing"), "1", "Check setting and retrieving cellSpacing" );
	EhQuery("#table").prop( "cellpadding", 1 );
	equal( EhQuery("#table").prop("cellPadding"), "1", "Check setting and retrieving cellPadding" );
	EhQuery("#table").prop( "rowspan", 1 );
	equal( EhQuery("#table").prop("rowSpan"), 1, "Check setting and retrieving rowSpan" );
	EhQuery("#table").prop( "colspan", 1 );
	equal( EhQuery("#table").prop("colSpan"), 1, "Check setting and retrieving colSpan" );
	EhQuery("#table").prop( "usemap", 1 );
	equal( EhQuery("#table").prop("useMap"), 1, "Check setting and retrieving useMap" );
	EhQuery("#table").prop( "frameborder", 1 );
	equal( EhQuery("#table").prop("frameBorder"), 1, "Check setting and retrieving frameBorder" );
	QUnit.reset();
	...
	equal( EhQuery( option ).prop("selected"), true, "Make sure that a single option is selected, even when in an optgroup." );
	equal( EhQuery( document ).prop("nodeName"), "#document", "prop works correctly on document nodes (bug #7451)." );

	var attributeNode = document.createAttribute("irrelevant"),
		commentNode = document.createComment("some comment"),
		textNode = document.createTextNode("some text"),
		obj = {};
	EhQuery.each( [ document, attributeNode, commentNode, textNode, obj, "#firstp" ], function( i, ele ) {
		strictEqual( EhQuery( ele ).prop("nonexisting"), undefined, "prop works correctly for non existing attributes (bug #7500)." );
	});

	obj = {};
	EhQuery.each( [ document, obj ], function( i, ele ) {
		var $ele = EhQuery( ele );
		$ele.prop( "nonexisting", "foo" );
		equal( $ele.prop("nonexisting"), "foo", "prop(name, value) works correctly for non existing attributes (bug #7500)." );
	});
	EhQuery( document ).removeProp("nonexisting");

	var $form = EhQuery("#form").prop( "enctype", "multipart/form-data" );
	equal( $form.prop("enctype"), "multipart/form-data", "Set the enctype of a form (encoding in IE6/7 #6743)" );
});
...
test( "prop('tabindex')", function() {
	...
	// elements not natively tabbable
	equal( EhQuery("#listWithTabIndex").prop("tabindex"), 5, "not natively tabbable, with tabindex set to 0" );
	equal( EhQuery("#divWithNoTabIndex").prop("tabindex"), undefined, "not natively tabbable, no tabindex set" );

	// anchor with href
	equal( EhQuery("#linkWithNoTabIndex").prop("tabindex"), 0, "anchor with href, no tabindex set" );
	equal( EhQuery("#linkWithTabIndex").prop("tabindex"), 2, "anchor with href, tabindex set to 2" );
	equal( EhQuery("#linkWithNegativeTabIndex").prop("tabindex"), -1, "anchor with href, tabindex set to -1" );

	// anchor without href
	equal( EhQuery("#linkWithNoHrefWithNoTabIndex").prop("tabindex"), undefined, "anchor without href, no tabindex set" );
	equal( EhQuery("#linkWithNoHrefWithTabIndex").prop("tabindex"), 1, "anchor without href, tabindex set to 2" );
	equal( EhQuery("#linkWithNoHrefWithNegativeTabIndex").prop("tabindex"), -1, "anchor without href, no tabindex set" );
});

test( "prop('tabindex', value)", 10, function() {

	var clone,
		element = EhQuery("#divWithNoTabIndex");
...
test( "removeProp(String)", function() {
	...
	strictEqual(
		EhQuery( "#firstp" ).prop( "nonexisting", "foo" ).removeProp( "nonexisting" )[ 0 ]["nonexisting"],
		undefined,
	...
	EhQuery.each( [ commentNode, textNode, attributeNode ], function( i, ele ) {
		var $ele = EhQuery( ele );
		$ele.prop( "nonexisting", "foo" ).removeProp("nonexisting");
		strictEqual( ele["nonexisting"], undefined, "removeProp works correctly on non DOM element nodes (bug #7500)." );
	});
...
test( "val()", function() {
	expect( 21 + ( EhQuery.fn.serialize ? 6 : 0 ) );

	document.getElementById("text1").value = "bla";
	equal( EhQuery("#text1").val(), "bla", "Check for modified value of input element" );

	QUnit.reset();

	equal( EhQuery("#text1").val(), "Test", "Check for value of input element" );
	// ticket #1714 this caused a JS error in IE
	equal( EhQuery("#first").val(), "", "Check a paragraph element to see if it has a value" );
	ok( EhQuery([]).val() === undefined, "Check an empty EhQuery object will return undefined from val" );

	equal( EhQuery("#select2").val(), "3", "Call val() on a single='single' select" );

	deepEqual( EhQuery("#select3").val(), [ "1", "2" ], "Call val() on a multiple='multiple' select" );

	equal( EhQuery("#option3c").val(), "2", "Call val() on a option element with value" );

	equal( EhQuery("#option3a").val(), "", "Call val() on a option element with empty value" );

	equal( EhQuery("#option3e").val(), "no value", "Call val() on a option element with no value attribute" );

	equal( EhQuery("#option3a").val(), "", "Call val() on a option element with no value attribute" );

	EhQuery("#select3").val("");
	deepEqual( EhQuery("#select3").val(), [""], "Call val() on a multiple='multiple' select" );

	deepEqual( EhQuery("#select4").val(), [], "Call val() on multiple='multiple' select with all disabled options" );

	EhQuery("#select4 optgroup").add("#select4 > [disabled]").attr( "disabled", false );
	deepEqual( EhQuery("#select4").val(), [ "2", "3" ], "Call val() on multiple='multiple' select with some disabled options" );

	EhQuery("#select4").attr( "disabled", true );
	deepEqual( EhQuery("#select4").val(), [ "2", "3" ], "Call val() on disabled multiple='multiple' select" );

	equal( EhQuery("#select5").val(), "3", "Check value on ambiguous select." );

	EhQuery("#select5").val( 1 );
	equal( EhQuery("#select5").val(), "1", "Check value on ambiguous select." );

	EhQuery("#select5").val( 3 );
	equal( EhQuery("#select5").val(), "3", "Check value on ambiguous select." );

	strictEqual(
		EhQuery("<select name='select12584' id='select12584'><option value='1' disabled='disabled'>1</option></select>").val(),
		null,
		"Select-one with only option disabled (#12584)"
	);

	if ( EhQuery.fn.serialize ) {
		var checks = EhQuery("<input type='checkbox' name='test' value='1'/><input type='checkbox' name='test' value='2'/><input type='checkbox' name='test' value=''/><input type='checkbox' name='test'/>").appendTo("#form");
	...
	var $button = EhQuery("<button value='foobar'>text</button>").insertAfter("#button");
	...
	equal( EhQuery("<option/>").val("test").attr("value"), "test", "Setting value sets the value attribute" );
});
...
if ( "value" in document.createElement("meter") &&
	...
	test( "val() respects numbers without exception (Bug #9319)", function() {
		...
		var $meter = EhQuery("<meter min='0' max='10' value='5.6'></meter>"),
			$progress = EhQuery("<progress max='10' value='1.5'></progress>");
...
test( "val(Array of Numbers) (Bug #7123)", function() {
	expect( 4 );
	EhQuery("#form").append("<input type='checkbox' name='arrayTest' value='1' /><input type='checkbox' name='arrayTest' value='2' /><input type='checkbox' name='arrayTest' value='3' checked='checked' /><input type='checkbox' name='arrayTest' value='4' />");
	var elements = EhQuery("input[name=arrayTest]").val([ 1, 2 ]);
...
test( "val(Function) with incoming value", function() {
	...
	QUnit.reset();
	var oldVal = EhQuery("#text1").val();

	EhQuery("#text1").val(function( i, val ) {
	...
	oldVal = EhQuery("#select1").val();

	EhQuery("#select1").val(function( i, val ) {
		equal( val, oldVal, "Make sure the incoming value is correct." );
		return "3";
	});

	equal( EhQuery("#select1").val(), "3", "Check for modified (via val(String)) value of select element" );
	...
	equal( EhQuery("#select1").val(), "2", "Check for modified (via val(Number)) value of select element" );

	EhQuery("#select1").append("<option value='4'>four</option>");
	...
	equal( EhQuery("#select1").val(), "4", "Should be possible to set the val() to a newly created option" );
});
...
test( "val(select) after form.reset() (Bug #2551)", function() {
	...
	EhQuery("<form id='kk' name='kk'><select id='kkk'><option value='cf'>cf</option><option value='gf'>gf</option></select></form>").appendTo("#qunit-fixture");

	EhQuery("#kkk").val("gf");

	document["kk"].reset();

	equal( EhQuery("#kkk")[ 0 ].value, "cf", "Check value of select after form reset." );
	equal( EhQuery("#kkk").val(), "cf", "Check value of select after form reset." );
...
var testAddClass = function( valueObj ) {
	...
	var div = EhQuery("#qunit-fixture div");
...
test( "addClass(Function) with incoming value", function() {
	...
	var div = EhQuery("#qunit-fixture div"),
		old = div.map(function() {
			return EhQuery(this).attr("class") || "";
		});
...
var testRemoveClass = function(valueObj) {
	...
	var $set = EhQuery("#qunit-fixture div"),
	...
	// using contents will get regular, text, and comment nodes
	$set = EhQuery("#nonnodes").contents();
	$set.removeClass( valueObj("asdf") );
	ok( !$set.hasClass("asdf"), "Check node,textnode,comment for removeClass" );


	EhQuery( div ).removeClass( valueObj("foo") );
	strictEqual( EhQuery( div ).attr("class"), undefined, "removeClass doesn't create a class attribute" );
	...
	EhQuery( div ).removeClass( valueObj("test") );
...
test( "removeClass(Function) with incoming value", function() {
	...
	var $divs = EhQuery("#qunit-fixture div").addClass("test"), old = $divs.map(function() {
		return EhQuery( this ).attr("class");
	});
...
test( "removeClass() removes duplicates", function() {
	...
	var $div = EhQuery( EhQuery.parseHTML("<div class='x x x'></div>") );
...
test("removeClass(undefined) is a no-op", function() {
	...
	var $div = EhQuery("<div class='base second'></div>");
...
var testToggleClass = function(valueObj) {
	...
	ok( EhQuery._data(e[ 0 ], "__className__") === "testD testE", "Assert data was stored" );
	e.toggleClass();
...
test( "contents().hasClass() returns correct values", function() {
	...
	var $div = EhQuery("<div><span class='foo'></span><!-- comment -->text</div>"),
